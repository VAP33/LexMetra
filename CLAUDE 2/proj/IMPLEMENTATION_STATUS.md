...
## Session 3 — Live capture, package boundary vs PDP, calibration stub

Continued directly, no phase gating. Same repo as Session 2 (no separate
"canonical" repo was ever provided to diff against — confirmed with the
user that this was the same project under a different folder name).

### New this session (verified: unit-tested + exercised through the real
### `/live/analyze` endpoint against a real photo via TestClient)

1. **`package_boundary.py`** (new) — classical Canny+contour package
   localization, deliberately structurally separate from PDP estimation
   (`image_quality.estimate_pdp_bbox`, which is OCR-line-driven). A test
   (`test_package_boundary_vs_pdp_are_independent_concepts`) asserts the
   package-boundary source code contains no OCR/Tesseract reference, to
   keep this distinction from silently eroding later. Returns `None` (not
   a guess) when no confident contour is found.

2. **`measurement.py`** (new) — an honest calibration STUB, not real
   calibration CV. `measure_with_calibration()` is the single gate every
   future measurement caller must go through: with no calibration
   reference (the only case anything in this codebase can currently
   produce) it always returns `UNCERTAIN`/`value_mm=None`; it only returns
   `VERIFIED` given an explicitly supplied `calibration_reference_mm_per_pixel`,
   which nothing currently supplies. No fiducial/ArUco/scale-detection CV
   was implemented — that remains explicitly NOT IMPLEMENTED, not silently
   assumed.

3. **`live_capture.py`** (new) — `analyze_live_frame()`: OCR-free by
   default (quality + package boundary + framing guidance only), returns
   `CaptureAnalysis`/`CaptureGuidance`/`LiveReadiness` (NOT_READY /
   ALMOST_READY / READY). Explicitly reuses `image_quality.py` and
   `package_boundary.py` rather than re-implementing quality/framing logic
   — one shared vision core, not a third parallel pipeline. An optional
   `sample_ocr=True` path runs one cheap downscaled Tesseract pass purely
   for a coarse "is there readable text yet" hint string — it never
   returns extracted fields. Guidance list is sorted high→medium→low
   severity so a client can just render `guidance[0]`.
   Tests (`tests/test_live_capture.py`, 8/8 passing) assert, among other
   things, that live guidance text never contains "pass"/"fail"/
   "compliant"/"violation" — a direct, automated check of the safety rule
   "Live READY != legal compliance", not just a stated intention.

4. **`POST /live/analyze`** wired into `main.py` — takes an image, returns
   readiness + guidance + package bbox, no legal fields, no OCR fields
   unless `sample_ocr=true` is explicitly passed (and even then, only a
   hint string). **Verified end-to-end**: registered a user, logged in,
   and called this endpoint via `TestClient` with a real Bru-jar photo —
   got `readiness: READY`, package detected at 0.81 frame-fill-ratio,
   correct guidance.

### Explicitly NOT done / honestly unverified

- **Real live-camera FPS/latency**: there is no camera or mobile client in
  this environment. `analyze_live_frame()` is implemented and unit-tested
  as a pure single-image function; its throughput on an actual phone
  camera stream is UNVERIFIED — nothing exists here to measure it against.
  Do not treat "implemented" as "confirmed fast enough for a live UI" until
  it's actually run against a real camera feed.
- **Cylindrical/spherical package-boundary handling**: `package_boundary.py`
  is a flat 2D bounding-box heuristic; it does not model or exploit package
  geometry (still correctly out of scope per master spec Part 3/6 and this
  project's repeated "no research-grade geometry solving" instruction).
- **Real calibration CV** (fiducial detection, scale recovery): not
  implemented, as above — `measurement.py` is a safety gate, not a
  calibration system.
- Overlap detection / perspective-normalized multi-image mosaicking beyond
  the already-existing scoped split-field text reconstruction
  (`evidence_fusion.reconstruct_split_fields`) was not attempted this
  session — still flagged as research-grade/out-of-scope per prior
  sessions' decisions, unchanged.

### Test suite state
`pytest tests/ -q` against live PostgreSQL: **61 passed, 0 failed, 0
skipped** (53 from Session 2 + 8 new live-capture/package-boundary/
measurement tests). No prior test broken.
