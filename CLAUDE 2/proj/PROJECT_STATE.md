...
backend/
  ...
  live_capture.py         live single-frame readiness (NOT_READY/ALMOST_READY/READY), no OCR by default, no legal signal
  package_boundary.py     classical package bbox estimation, structurally separate from PDP (image_quality.estimate_pdp_bbox)
  measurement.py          calibration STUB - always UNCERTAIN without an explicit calibration reference; no real calibration CV implemented
...
| Live capture readiness | Working (new, Session 3). `POST /live/analyze` — OCR-free quality/package/framing check, returns NOT_READY/ALMOST_READY/READY + guidance. Verified end-to-end via TestClient against a real photo. Real-camera FPS/latency is **unverified** — no camera/mobile client exists in this environment to measure it against. |
| Package boundary vs PDP | Working (new). `package_boundary.py` is classical Canny/contour, structurally independent of PDP's OCR-line-driven estimate — enforced by a test that greps the source for OCR references. |
| Measurement/calibration | Stub only (new, honest). Always reports UNCERTAIN without an explicit calibration reference; no fiducial/scale-detection CV implemented. |
| Tests | Working. 61 tests, 0 skipped, unit + live-API integration, verified against a real Postgres 16 instance.
