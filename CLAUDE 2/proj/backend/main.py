...
# ---------------------------------------------------------------------------
# Live capture readiness (lightweight, NO OCR by default, NO legal decision)
# ---------------------------------------------------------------------------

@app.post("/live/analyze")
async def live_analyze(
    file: UploadFile = File(...),
    sample_ocr: bool = Form(False),
    current_user: auth.CurrentUser = Depends(auth.require_inspector),
):
    """
    Frame-readiness check for a live capture UI. Deliberately cheap: quality
    + package boundary + framing guidance, no OCR by default. NEVER returns
    a legal signal - "READY" means "good enough to capture evidence", not
    "compliant". Use POST /scan or /sessions/{id}/captures for the full
    (expensive) pipeline once the inspector actually taps capture.
    """
    _raw_bytes, img, _pil_img = await _read_image_upload(file)
    analysis = live_capture.analyze_live_frame(img, sample_ocr=sample_ocr)

    return {
        "readiness": analysis.readiness.value,
        "package_detected": analysis.package_detected,
        "package_frame_fill_ratio": analysis.package_frame_fill_ratio,
        "global_quality_status": analysis.global_quality_status,
        "detected_regions": [
            {"name": r.name, "bbox": list(r.bbox), "confidence": r.confidence}
            for r in analysis.detected_regions
        ],
        "guidance": [
            {
                "state": g.state.value,
                "message": g.message,
                "target_region": g.target_region,
                "severity": g.severity,
                "actions": g.actions,
            }
            for g in analysis.guidance
        ],
        "sampled_ocr_hint": analysis.sampled_ocr_hint,
    }
