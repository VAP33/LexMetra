...
def _large_sparse_fused_blob(width: int = 463, height: int = 233) -> np.ndarray:
    """
    ONE 8-connected component whose bounding box covers ~95% of the crop
    while its own fill density is only ~11% — built directly (a thin zigzag
    spanning the full width and height) rather than by hoping a rendered
    multi-line paragraph happens to fuse the same way, because whether text
    lines fuse under morphological closing is itself sensitive to font
    rendering and antialiasing, which is exactly the kind of environment
    dependency this suite tries not to rely on for a P0 regression test.

    This is the general shape of the live defect found in
    `prod001_compliant.png`'s declaration panel: closing that panel's 8
    stacked lines (`Manufacturer:` ... `Consumer Care:`) along the horizontal
    axis fused them into one component whose bounding box spanned nearly the
    whole region, at roughly this same low fill density.
    """
    binary = np.zeros((height, width), np.uint8)
    pts = []
    n = 8
    for i in range(n):
        y = int(5 + i * (height - 10) / (n - 1))
        x0, x1 = (5, width - 5) if i % 2 == 0 else (width - 5, 5)
        pts.append((x0, y))
        pts.append((x1, y))
    pts_arr = np.array(pts, dtype=np.int32).reshape(-1, 1, 2)
    cv2.polylines(binary, [pts_arr], isClosed=False, color=255, thickness=2)
    return binary


def test_a_large_sparse_blob_is_not_treated_as_a_filled_graphic():
    """
    Regression test for the live P0 defect: a real multi-line declaration
    panel (`prod001_compliant.png`) was rotated 90 degrees before OCR because
    `_merge_elongation` discarded its only horizontal blob as a "filled
    graphic" purely because its bounding box exceeded 85% of the region —
    without checking whether that blob was actually filled. A bounding box
    that large but only ~11% inked is what a fused multi-line declaration
    panel looks like, not a solid graphic, and must not be discarded.
    """
    binary = _large_sparse_fused_blob()
    h_score = ori._merge_elongation(binary, axis="horizontal", kernel_len=3)
    assert h_score > 0.0, (
        "The horizontal score collapsed to zero, meaning the one large, "
        "sparse blob was discarded as a 'filled graphic' — reintroducing "
        "the bounding-box-only discard rule fails this assertion. This is "
        "the exact mechanism that rotated a legible declaration panel into "
        "unreadable OCR input in the live pipeline."
    )


def test_filled_graphic_is_still_excluded_from_the_elongation_score():
    """
    Non-vacuity check for the density fix above: a genuinely solid block
    (a filled rectangle, not fused text) must still be excluded so a dark
    logo or swatch cannot manufacture a false axis reading. Without this
    test, loosening the discard rule to fix the paragraph case above could
    silently break the case it was written for.
    """
    # Built directly as a binary ink mask (bypassing `_ink_mask`'s Otsu +
    # minority-class polarity normalisation, which is a separate concern) so
    # this test exercises exactly what `_merge_elongation` decides: one
    # solid, fully-inked component whose own bounding box covers >85% of the
    # crop AND is >85% filled within that box — the actual "filled graphic"
    # case the discard rule exists for.
    binary = np.zeros((233, 463), np.uint8)
    binary[5:228, 5:458] = 255
    h_score = ori._merge_elongation(binary, axis="horizontal", kernel_len=14)
    v_score = ori._merge_elongation(binary, axis="vertical", kernel_len=14)
    assert h_score == 0.0
    assert v_score == 0.0
