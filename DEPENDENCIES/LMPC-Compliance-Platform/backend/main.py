...
from schema import FactStatus, GeometryType, MeasurementMode, ProductInspection, SurfaceObservation
...
def _decode_barcodes(images_for_barcode: List[tuple]) -> barcode_decode.SymbolReadResult:
    """
    Run barcode/QR decoding across every surface of one inspection.

    Wraps barcode_decode.decode_across_images() so both /extract-preview and
    /scan share one call site. Never raises: a decode failure degrades to
    NO_SYMBOL_LOCALISED, exactly like "OCR found nothing" does elsewhere in
    this file, rather than 500ing the whole request.
    """
    if not images_for_barcode:
        return barcode_decode.SymbolReadResult(
            image_id="MULTI_SURFACE", status=barcode_decode.SymbolStatus.NO_SYMBOL_LOCALISED
        )
    try:
        return barcode_decode.decode_across_images(images_for_barcode)
    except Exception as exc:  # pragma: no cover - defensive, mirrors OCR's degrade-gracefully stance
        result = barcode_decode.SymbolReadResult(
            image_id="MULTI_SURFACE", status=barcode_decode.SymbolStatus.NO_SYMBOL_LOCALISED
        )
        result.notes.append(f"Barcode decoding failed: {exc}")
        return result


def _detect_geometry(
    first_img_np: Optional[np.ndarray],
    first_image_id: Optional[str],
    all_ocr_lines: List[Any],
) -> tuple:
    """
    Detect package + PDP boundary geometry for the first captured surface.

    This is boundary/shape evidence only (contour + OCR-box based). It does
    NOT produce a calibrated pdp_area_cm2 -- that still requires a real-world
    reference (coin/card in frame, known package dimension, or a depth
    reading), none of which this app currently captures. See anti2.md S2.
    Never raises: geometry evidence is advisory, not required for a scan to
    complete.
    """
    if first_img_np is None or first_image_id is None:
        return None, None
    try:
        package_geometry = geometry_module.detect_package_geometry(
            first_img_np, source_image=first_image_id,
        )
        pdp_geometry = geometry_module.detect_pdp_geometry(
            first_img_np,
            source_image=first_image_id,
            package_geometry=package_geometry,
            ocr_boxes=[line.bbox for line in all_ocr_lines if getattr(line, "bbox", None)],
        )
        return package_geometry, pdp_geometry
    except Exception:  # pragma: no cover - defensive, geometry must never break a scan
        return None, None


def _infer_suggested_category(accumulated_fields: Dict[str, dict], all_ocr_lines: List[Any]) -> str:
    """Infer product category from extracted text keywords."""
    combined_text = " ".join(
        [str(v.get("value", "")) for v in accumulated_fields.values() if isinstance(v, dict)]
        + [str(getattr(l, "text", "")) for l in all_ocr_lines]
    ).lower()

    if any(w in combined_text for w in ["shampoo", "soap", "cream", "lotion", "serum", "hair", "skin", "oil", "sunscreen", "toothpaste", "cosmetic"]):
        return "personal_care"
    if any(w in combined_text for w in ["detergent", "cleaner", "wash", "dishwash", "disinfectant", "floor", "bleach", "freshener"]):
        return "household"
    if any(w in combined_text for w in ["juice", "tea", "coffee", "drink", "water", "beverage", "soda", "syrup", "cola"]):
        return "beverage"
    if any(w in combined_text for w in ["biscuit", "cookie", "flour", "rice", "atta", "snack", "noodle", "spice", "masala", "grain", "pulse", "wheat", "food", "milk", "butter", "cheese", "paneer"]):
        return "food"
    return "other"
...
async def extract_preview(
    ...
    for i, upload in enumerate(upload_list):
        ...
        if first_img_np is None:
            first_img_np = img
            first_image_id = image_id
        images_for_barcode.append((image_id, img))
    ...
    barcode_result = _decode_barcodes(images_for_barcode)
    package_geometry, pdp_geometry = _detect_geometry(first_img_np, first_image_id, all_ocr_lines)
    ...
    # Priority: a decoded barcode/GTIN (real identity, ahead of guessed text)
    # -> common_name -> manufacturer_name -> an explicitly-flagged placeholder.
    # A CONFLICTING barcode read (two different codes across surfaces) is
    # never used as an identity -- see barcode_decode.SymbolReadResult.best.
    ...
    suggested_pid = ""
    id_source = "unidentified_placeholder"
    id_needs_confirmation = True

    best_symbol = barcode_result.best
    if barcode_result.status == barcode_decode.SymbolStatus.DECODED and best_symbol and best_symbol.gtin13:
        suggested_pid = f"PROD-{best_symbol.gtin13}"
        id_source = "barcode_scan" if best_symbol.method == barcode_decode.ReadMethod.CV_BARS_DECODED else "barcode_ocr_checked"
        id_needs_confirmation = bool(best_symbol.needs_confirmation)
    if not suggested_pid and c_name:
        clean_c = re.sub(r'[^a-zA-Z0-9]+', '-', c_name).strip('-')[:25]
        if clean_c:
            suggested_pid = f"PROD-{clean_c.upper()}"
            id_source = "common_name"
    if not suggested_pid and m_name:
        clean_m = re.sub(r'[^a-zA-Z0-9]+', '-', m_name).strip('-')[:20]
        if clean_m:
            suggested_pid = f"PROD-{clean_m.upper()}"
            id_source = "manufacturer_name"
    ...
    return {
        ...
        "suggested_details": {
            ...
            "product_id_source": id_source,
            "product_id_needs_confirmation": id_needs_confirmation,
            "sale_type": "retail",
        ...
        "barcode": barcode_result.to_dict(),
        "geometry": {
            "package": package_geometry.model_dump() if package_geometry else None,
            "pdp": pdp_geometry.model_dump() if pdp_geometry else None,
        },
    }
...
async def scan(
    ...
    # ------------------------- Barcode / geometry evidence ----------------
    barcode_result = _decode_barcodes(images_for_barcode)
    package_geometry, pdp_geometry = _detect_geometry(first_img_np, first_image_id, all_ocr_lines)

    # Resolve product_id fallback.
    # Priority when the inspector didn't type one: a decoded barcode/GTIN
    # (real identity) -> common_name -> manufacturer_name -> a placeholder
    # that is explicitly NOT presented as a successful identification.
    resolved_product_id = product_id.strip() if (product_id and product_id.strip()) else ""
    product_id_source = "request" if resolved_product_id else ""
    if not resolved_product_id:
        best_symbol = barcode_result.best
        if barcode_result.status == barcode_decode.SymbolStatus.DECODED and best_symbol and best_symbol.gtin13:
            resolved_product_id = f"PROD-{best_symbol.gtin13}"
            product_id_source = "barcode_scan" if best_symbol.method == barcode_decode.ReadMethod.CV_BARS_DECODED else "barcode_ocr_checked"
        if not resolved_product_id:
            c_name = accumulated_fields.get("common_name", {}).get("value")
            m_name = accumulated_fields.get("manufacturer_name", {}).get("value") or accumulated_fields.get("manufacturer_name_address", {}).get("value")
            if c_name:
    ...
    result = run_inspection(
        ...
        geometry=package_geometry.shape if package_geometry else GeometryType.UNKNOWN,
    )
