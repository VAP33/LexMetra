  ...
  useEffect(() => {
    ...
    async function runPreview() {
      ...
      try {
        ...
        // Only pre-fill the product ID field when it's a real identification
        // (a decoded barcode, or text plausibly naming the product) -- never
        // for the "unidentified_placeholder" fallback, which is a generated
        // string, not a read value, and should never masquerade as one.
        if (
          res.suggested_details.product_id &&
          res.suggested_details.product_id_source !== "unidentified_placeholder"
        ) {
          ...
          <div className="mt-6 space-y-5">
            <div>
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  const src = previewData?.suggested_details?.product_id_source;
                  if (src === "barcode_scan") {
                    return (
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-600 dark:text-emerald-400">
                        <Sparkles className="h-3 w-3" /> Read from barcode
                      </span>
                    );
                  }
                  if (src === "barcode_ocr_checked") {
                    return (
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-600 dark:text-amber-400">
                        <Sparkles className="h-3 w-3" /> Barcode digits (please confirm)
                      </span>
                    );
                  }
                  if (src === "common_name" || src === "manufacturer_name") {
                    return (
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-600 dark:text-emerald-400">
                        <Sparkles className="h-3 w-3" /> Auto-suggested from label text
                      </span>
                    );
                  }
                  if (src === "unidentified_placeholder") {
                    return (
                      <span className="text-[11px] font-medium text-muted-foreground">
                        Not identified — type manually
                      </span>
                    );
                  }
                  return null;
      ...
      <main className="mx-auto max-w-5xl space-y-5 px-4 pb-28 pt-6 sm:px-6 md:pb-10 lg:px-8 lg:pt-10">
        ...
        <section className={`overflow-hidden rounded-2xl border ${style.border} ${style.bg}`}>
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-current/10 bg-card/50 p-5 sm:grid-cols-4">
            ...
            <div><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Checked</p><p className="mt-1 text-sm font-semibold">{inspection.declarationSummary ? `${inspection.declarationSummary.verified} / ${inspection.declarationSummary.applicable}` : `${verified} / ${total}`}</p></div>
