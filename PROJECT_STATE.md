...
> ### ⚠ PARTLY SUPERSEDED BY SESSIONS 3 AND 4 — read this first
...
> four respects. See `IMPLEMENTATION_STATUS.md` §§ "Session 3" and "Session 4".
>
...
> 3. **Test counts here are stale.** The suite is now 249 passed / 1 collection
...
> 4. **Four evidence-traceability defects were fixed in Session 4, and two rows
>    of the table below were simply wrong.** Findings were never persisted — there
>    was no `inspection_findings` table — and every fact the live API produced
>    carried an empty evidence list because the OCR bounding box was dropped one
>    call before the rule engine. Consequently the PDF report rendered extracted
>    *facts* under a "Rule Findings" heading while asserting each row was
>    traceable to a versioned rule. **Any report generated before Session 4 is
>    unreliable as a record of evaluation.** Where §1.1 below says PDF reports and
>    the `/scan` pipeline are "Working", read that as "runs without error", not
>    "records the legal findings".
>
