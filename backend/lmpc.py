...
class LMPCModule(RegulatoryModule):
    ...
    def applicability(self, context: RegulatoryContext) -> Dict[str, Any]:
        if context.sale_type in {"industrial", "institutional"}:
            return {
                "status": "CONDITIONAL",
                "reason": "Direct industrial/institutional treatment requires the exact package and sale conditions to be established.",
                "rule_ids": ["LMPC-2011-R3-SCOPE"],
            }
        return {
            "status": "APPLICABLE",
            "reason": "LMPC is the configured primary packaged-commodity module for this inspection.",
            "rule_ids": ["LMPC-2011-R3-SCOPE"],
        }
        ...
        for rule in data.get("rules", []):
            for item in rule.get("requirements", []):
                ...
                result.append(EvidenceRequirement(
                    ...
                    description=str(item.get("description") or rid),
                    condition=item.get("condition"),
                ))
        return result

    def evaluate(self, context: RegulatoryContext, evidence: Dict[str, Any]) -> List[RegulatoryFinding]:
        # The actual evaluator is deliberately not duplicated here. Callers
        # should invoke rule_engine.run_inspection() with typed evidence and use
        # this module for platform-level applicability/metadata.
        raise RuntimeError(
            "LMPCModule.evaluate is intentionally delegated to the existing "
            "deterministic rule_engine. Use regulatory.lmpc_adapter.run_lmpc()."
        )
    ...
    def explain(self, finding: RegulatoryFinding) -> Dict[str, Any]:
        return {
            ...
            "warning": "Explanation is subordinate to the deterministic finding and does not change it.",
        }
