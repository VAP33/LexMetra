...
These are interfaces, not fake compliance engines. They report UNKNOWN until
real domain rules and approved sources are registered.
"""
...
class UnsupportedRegulatoryModule(RegulatoryModule):
    def __init__(self, module_id: str, name: str, department: str, regulation: str) -> None:
        self._meta = RegulatoryModuleMetadata(
            id=module_id,
            name=name,
    ...
    def supported_context(self) -> Dict[str, Any]:
        return {"common_regulatory_context": True}

    def applicability(self, context: RegulatoryContext) -> Dict[str, Any]:
        return {"status": "UNKNOWN", "reason": "No approved implementation is active for this module."}
    ...
    def explain(self, finding: RegulatoryFinding) -> Dict[str, Any]:
        return {"status": "UNKNOWN", "reason": "Module interface only; no compliance verdict is generated."}
