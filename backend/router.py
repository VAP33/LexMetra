...
These routes expose legal knowledge retrieval separately from compliance
inspection. Retrieval can explain rules, but it cannot change inspection status.
"""
...
from regulatory import default_registry
from regulatory.api_models import RAGQueryRequest
...
def retrieve(req: RAGQueryRequest, current_user: auth.CurrentUser = Depends(auth.require_inspector)):
    ...
    return {
        "query": req.query,
        "effective_date": req.inspection_date,
        "module": req.module,
        "hits": [
            {
                "chunk_id": h.chunk.id,
...
async def ingest_knowledge(
    ...
    """Ingest a source document into the knowledge index without activating law."""
    if not file.filename or not file.filename.lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Only PDF regulatory source documents are accepted.")

    payload = await file.read()
    if not payload:
        raise HTTPException(status_code=400, detail="The uploaded regulatory document is empty.")
    ...
    try:
        chunks = ingest_pdf(
            ...
            index_version=f"{document_id}:{document_version}",
    ...
    # Persist the source chunks. Activation of a new legal rule version remains
    # a separate, human-approved lifecycle and is never implied by ingestion.
    ...
    return {
        "status": "INGESTED_PENDING_ACTIVATION",
        "document_id": document_id,
        "document_version": document_version,
        "chunks": len(chunks),
