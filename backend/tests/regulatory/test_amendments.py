...
def draft():
    return AmendmentDraft(
        ...
        extracted_at=datetime(2026, 9, 9),
    )


def test_amendment_requires_review_before_approval():
    with pytest.raises(ValueError):
        transition_amendment(draft(), ApprovalState.APPROVED)


def test_amendment_transition_is_explicit():
    d = transition_amendment(draft(), ApprovalState.EXTRACTED)
    d = transition_amendment(d, ApprovalState.AI_PARSED)
    d = transition_amendment(d, ApprovalState.PENDING_REVIEW)
    d = transition_amendment(d, ApprovalState.APPROVED)
