...
def test_expired_rule_is_not_selected():
    selected = select_rule_version(
        [
            rv("old", "2023", "2023-01-01", "2025-01-01"),
            rv("new", "2025", "2025-01-01"),
        ],
        module="lmpc",
        rule_id="R6",
        inspection_date=date(2026, 9, 9),
...
def test_unapproved_rule_is_not_selected():
    try:
        select_rule_version(
            ...
            module="lmpc",
            rule_id="R6",
            inspection_date=date(2027, 2, 1),
        )
    except RuleVersionSelectionError:
        return
    raise AssertionError("unapproved rule version must not be selected")
